...
void setup() {
	...
	logger.println(F("SatelliteTracker.ino"));
	logger.println(F("Monitoring satellite location and signal strength using TinyGPSCustom"));
...
void loop() {
  ...
  logger.println("BEGIN LOOP");
  logger.println("ss.available()" +(String)ss.available());
	if (ss.available() > 0) {
		gps.encode(ss.read());
		if (totalGPGSVMessages.isUpdated()) {
			for (int i=0; i<4; ++i) {
				int no = atoi(satNumber[i].value());
				// logger.print(F("SatNumber is ")); logger.println(no);
			...
			if (totalMessages == currentMessage) {
				logger.print(F("Sats=")); logger.print(gps.satellites.value());
				logger.print(F(" Nums="));
				...
				logger.print(F(" Elevation="));
				for (int i=0; i<MAX_SATELLITES; ++i) {
					if (sats[i].active) {
						logger.print(sats[i].elevation);
						logger.print(F(" "));
					}
				}
				logger.print(F(" Azimuth="));
				for (int i=0; i<MAX_SATELLITES; ++i) {
					if (sats[i].active) {
						logger.print(sats[i].azimuth);
						logger.print(F(" "));
					}
				}
				logger.print(F(" SNR="));
