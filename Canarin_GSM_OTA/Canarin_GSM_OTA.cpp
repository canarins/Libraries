...
static Canarin_GSM_OTA * _esp_ota_obj_ptr = 0;
...
Canarin_GSM_OTA::Canarin_GSM_OTA()
:_client{nullptr}
,_ota_header{0}
,_ota_size(0)
,_crc32(0)
...
Canarin_GSM_OTA::Error Canarin_GSM_OTA::begin()
{
...
void Canarin_GSM_OTA::setCACert (const char *rootCA)
{
  if(rootCA != nullptr) {
    _ca_cert = rootCA;
  }
...
uint8_t Canarin_GSM_OTA::read_byte_from_network()
{
  bool is_http_data_timeout = false;
  for(unsigned long const start = millis();;)
  {
    is_http_data_timeout = (millis() - start) > CANARIN_GSM_OTA_BINARY_BYTE_RECEIVE_TIMEOUT_ms;
...
void Canarin_GSM_OTA::write_byte_to_flash(uint8_t data)
{
  Update.write(&data, 1);
}

int Canarin_GSM_OTA::download(const char * ota_url, TinyGsmClient * client)
{
  URI url(ota_url);
  int port = 0;
  ...
  if (url.protocol_ == "http") {
    ...
    _client = client;
    static_cast<TinyGsmClientSecure*>(_client)->setCertificate(_ca_cert);
    port = 443;
  } else {
  ...
  {
    is_http_header_timeout = (millis() - start) > CANARIN_GSM_OTA_HTTP_HEADER_RECEIVE_TIMEOUT_ms;
  ...
  if (_ota_header.header.magic_number != CANARIN_GSM_OTA_MAGIC)
  {
...
Canarin_GSM_OTA::Error Canarin_GSM_OTA::update()
{
  /* ... then finalise ... */
  _crc32 ^= 0xFFFFFFFF;
